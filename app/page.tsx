...
const NeuralSphere = dynamic(() => import("@/components/NeuralSphere").then(m => ({ default: m.NeuralSphere })), {
  ssr: false,
...
function ServicesSection() {
  ...
  const slides = [
    { category: "Web Development", badge: "Scale-Ready", statValue: "99%", statSub: "Lighthouse Speed Score", highlightText: "High-performance React architectures", subText: "built with TanStack and Next.js to maximize conversion rates and compounding user engagement.", iconText: "React / Node / TS", color: "#1800AD" },
    { category: "Web Design", badge: "Conversion-Led", statValue: "40%", statSub: "Average Conversion Lift", highlightText: "Bespoke user-focused interfaces", subText: "designed in Figma around high-intent buyer journeys and tailored brand aesthetics.", iconText: "UX / Prototyping", color: "#0EA5A4" },
    { category: "AI Automation", badge: "Efficiency Labs", statValue: "18h", statSub: "Saved per Team Sprint", highlightText: "Custom autonomous agent pipelines", subText: "engineered with n8n and Make to eliminate manual operational bottlenecks 24/7.", iconText: "n8n / LLM / Workflows", color: "#1800AD" },
    { category: "Web Chatbots", badge: "Automation", statValue: "94%", statSub: "First-Response Accuracy", highlightText: "Intelligent customer service agents", subText: "integrating custom vector databases (RAG) over your support docs for immediate solutions.", iconText: "OpenAI / Pinecone", color: "#0EA5A4" },
    { category: "Voice Bots", badge: "Operations", statValue: "2.8x", statSub: "Booking Velocity Increase", highlightText: "Natural real-time voice agents", subText: "qualified for inbound customer inquiries, call scheduling, and automated reminders.", iconText: "Vapi / Twilio", color: "#1800AD" },
    { category: "App Development", badge: "Mobile Apps", statValue: "4.9★", statSub: "Average Store Rating", highlightText: "Refined native mobile platforms", subText: "compiled with React Native and Flutter for lightning-fast iOS & Android performance.", iconText: "Flutter / Native", color: "#0EA5A4" },
    { category: "SEO Systems", badge: "Acquisition", statValue: "340%", statSub: "Organic Traffic Increase", highlightText: "Technical and content SEO systems", subText: "wired to acquisition pipelines and compounding organic leads that scale without paid ad spend.", iconText: "Audits / Backlinks", color: "#1800AD" },
    { category: "Paid Marketing", badge: "Growth Engine", statValue: "4.2x", statSub: "Average Ad Spend ROI", highlightText: "Full-funnel traffic management", subText: "combining landing page testing and programmatic ads to acquire high-value customers.", iconText: "Meta / Google / Ads", color: "#0EA5A4" },
    { category: "Social Media Handling", badge: "Brand Voice", statValue: "10x", statSub: "Audience Growth Rate", highlightText: "Consistency-first community growth", subText: "leveraging high-quality custom asset creation and full-service community handling pipelines.", iconText: "Creatives / Handling", color: "#1800AD" },
  ];
  ...
  return (
    <section ref={ref} id="home-services-section" className="section w-full py-20 bg-slate-50 flex items-center justify-center">
      <div className="container-x max-w-6xl w-full">
        <div className="flex flex-wrap items-end justify-between gap-6 mb-10">
          <div className="transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)" }}>
        ...
        >
          ...
          <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-6">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono tracking-widest uppercase px-3 py-1.5 rounded-full bg-white/10 border border-white/15">{current.iconText}</span>
              <span className="text-[11px] font-bold px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/10">{current.badge}</span>
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              {slides.map((_, sIdx) => (
                <button
                  ...
                  style={{ width: sIdx === activeSlide ? "32px" : "10px", background: sIdx === activeSlide ? "#0EA5A4" : "rgba(255,255,255,0.25)" }}
                  aria-label={`Go to slide ${sIdx + 1}`}
                />
              ))}
            </div>
            <Link href="/contact" className="inline-flex items-center gap-2 rounded-xl bg-white text-[#1800AD] px-5 py-2.5 text-xs font-black uppercase tracking-wider transition-all hover:bg-slate-100 hover:scale-105 shadow-sm">
              Start Project &rarr;
            </Link>
          </div>
        </div>
...
function WhyChooseSection() {
  ...
  return (
    <section ref={ref} className="w-full min-h-screen lg:h-screen flex items-center py-10 lg:py-0" style={{ background: "radial-gradient(circle at 5% 10%, rgba(24,0,173,0.03) 0%, transparent 50%), radial-gradient(circle at 95% 90%, rgba(14,165,164,0.03) 0%, transparent 50%), #FAF9F6", borderTop: "1px solid rgba(24,0,173,0.05)", borderBottom: "1px solid rgba(24,0,173,0.05)" }}>
      <div className="container-x w-full py-10">
        <div className="grid items-center gap-12 lg:grid-cols-12">
          <div className="transition-all duration-700 lg:col-span-5" style={{ opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(-20px)" }}>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/5 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-primary">Why Vexcort</span>
            ...
            <p className="mt-4 text-[#475569] text-sm leading-relaxed max-w-xl">
              We are not a content shop or a freelance marketplace. We are a focused team that ships premium digital products and measurable growth systems — delivering strategy, design, engineering, and growth under one roof.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/about" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0B1324] px-5 py-3 text-xs font-bold text-white transition-all duration-300 hover:bg-[#0B1324]/90 hover:scale-[1.02] shadow-md shadow-slate-950/10">
                About Vexcort <ArrowUpRight className="h-3.5 w-3.5" />
              </Link>
              <Link href="/contact" className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 px-5 py-3 text-xs font-bold text-slate-800 transition-all duration-300 hover:border-slate-300">
                Get in touch
              </Link>
            </div>
            <div className="mt-8 grid grid-cols-2 gap-4">
              {stats.map((s, i) => (
                <div key={s.label} className="rounded-2xl p-4 transition-all duration-700 hover:scale-[1.02]" style={{ background: "rgba(255,255,255,0.8)", borderLeft: i % 2 === 0 ? "4px solid #1800AD" : "4px solid #0EA5A4", borderTop: "1px solid rgba(24,0,173,0.06)", borderRight: "1px solid rgba(24,0,173,0.06)", borderBottom: "1px solid rgba(24,0,173,0.06)", boxShadow: "0 10px 25px -10px rgba(24,0,173,0.06)", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(20px)", transition: "all 0.5s cubic-bezier(0.16, 1, 0.3, 1)", transitionDelay: `${300 + i * 60}ms` }}>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:col-span-7">
            {[
              { num: "01", title: "Senior craft, end-to-end", desc: "A focused senior team handles strategy, design, engineering and growth — one roof, zero handoffs.", icon: Globe, accent: "#1800AD" },
              { num: "02", title: "Ship in weeks, not quarters", desc: "Tight sprints, weekly demos, real product in your hands fast. No agency bloat.", icon: Zap, accent: "#0EA5A4" },
              { num: "03", title: "Built to scale safely", desc: "Modern stacks, type-safe code, security and performance baked in from day one.", icon: ShieldCheck, accent: "#1800AD" },
              { num: "04", title: "Measurable outcomes", desc: "Every engagement ties back to revenue, retention or efficiency. No vanity metrics.", icon: BarChart3, accent: "#0EA5A4" },
            ].map(({ num, title, desc, icon: Icon, accent }, i) => (
              <div key={num} className="group relative flex flex-col justify-between rounded-2xl p-6 transition-all duration-500 hover:-translate-y-1" style={{ background: "#FFFFFF", border: "1px solid rgba(24,0,173,0.07)", boxShadow: "0 10px 30px -15px rgba(24,0,173,0.05)", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(25px)", transitionDelay: `${i * 80}ms` }}>
                <div className="absolute top-0 left-0 right-0 h-[3px] rounded-t-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: `linear-gradient(90deg, ${accent}, #0EA5A4)` }} />
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center justify-center h-10 w-10 rounded-xl transition-all duration-300 group-hover:scale-110" style={{ background: `${accent}10`, color: accent }}>
                      <Icon className="h-5 w-5" />
                    </div>
                    <span className="text-xs font-bold text-slate-300 tracking-wider font-mono">/{num}</span>
                  </div>
                  <h3 className="text-sm font-bold text-[#0B1324] group-hover:text-primary transition-colors duration-300">{title}</h3>
                  <p className="mt-2 text-xs text-[#64748B] leading-relaxed">{desc}</p>
                </div>
              </div>
            ))}
          </div>
...
function BentoGrid() {
  ...
  return (
    <section ref={ref} className="section w-full" style={{ background: "linear-gradient(180deg,#fff 0%,#F4F6FF 100%)" }}>
      <div className="container-x">
        <div className="mb-12 transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(30px)" }}>
          <span className="eyebrow">Work</span>
          <h2 className="h-display mt-3 text-4xl md:text-5xl max-w-2xl">Digital Products &amp; Services We Ship for Ambitious Brands</h2>
        </div>
        <div className="grid md:grid-cols-12 gap-5 items-stretch transition-all duration-700" style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(40px)", transitionDelay: "150ms" }}>
          <div className="md:col-span-5 relative rounded-3xl overflow-hidden min-h-[380px] group shadow-sm hover:shadow-xl transition-all duration-500">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-7 text-white">
              ...
              <Link href="/services" className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-white/15 border border-white/25 backdrop-blur-md px-4 py-1.5 text-xs font-semibold hover:bg-white hover:text-slate-900 transition-all">
          ...
          <div className="md:col-span-3 flex flex-col gap-5">
            ...
            <div className="rounded-3xl p-6 flex flex-col justify-center min-h-[160px]" style={{ background: "rgba(24,0,173,0.05)", border: "1px solid rgba(24,0,173,0.12)" }}>
...
export default function Home() {
  const websiteSchema = {
    ...
    potentialAction: { "@type": "SearchAction", target: "https://vexcort.com/search?q={search_term_string}", "query-input": "required name=search_term_string" },
  };
